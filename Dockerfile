...
#COPY --from=base /data/drone/drone-server-oss /root/drone-server-oss
FROM centos:7
RUN  rm -rf /etc/yum.repos.d/* && curl -o /etc/yum.repos.d/CentOS-Base.repo https://mirrors.aliyun.com/repo/Centos-7.repo  &&yum install epel-release && yum localinstall --nogpgcheck https://download1.rpmfusion.org/free/el/rpmfusion-free-release-7.noarch.rpm && yum install ffmpeg ffmpeg-devel && ffmpeg -version
