...
RUN  sed -i 's/dl-cdn.alpinelinux.org/mirrors.aliyun.com/g' /etc/apk/repositories && apk add gcc build-base musl-dev python3-dev libffi-dev && pip install --upgrade pip && pip install cryptography==3.1.1 Cython matplotlib==3.3.2 numpy==1.19.2 pandas==1.1.3 PyMySQL==1.0.2 scikit_learn==0.23.2 scipy==1.5.2 setuptools==58.0.4 setuptools==50.3.1.post20201107 SQLAlchemy==1.3.20 statsmodels==0.12.0
