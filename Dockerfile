...
FROM maven
SHELL /bin/bash
RUN apt update>/dev/null && apt install -qq -y rpm yarn nodejs npm curl python3 python3-dev python3-pip python3-full>/dev/null && ln -s /usr/bin/python3 /usr/bin/python && curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | tee /etc/apt/trusted.gpg.d/yarn.asc  && echo "deb https://dl.yarnpkg.com/debian/ stable main" | tee /etc/apt/sources.list.d/yarn.list && apt update && apt install -qq -y yarn openjdk-8-jdk > /dev/null 2>&1 && rm -rf /opt/java/openjdk  && cp -a /usr/lib/jvm/java-8-openjdk-amd64 /opt/java/openjdk && wget -q https://www-eu.apache.org/dist/ambari/ambari-2.7.8/apache-ambari-2.7.8-src.tar.gz && tar xfz apache-ambari-2.7.8-src.tar.gz && cd apache-ambari-2.7.8-src && mvn -q versions:set -DnewVersion=2.7.8.0.0 > /dev/null 2>&1 && pushd ambari-metrics && mvn -q versions:set -DnewVersion=2.7.8.0.0 > /dev/null 2>&1 && popd && echo 1111111111111111111111111111111111111111 && mvn -v -B clean install rpm:rpm -DnewVersion=2.7.8.0.0 -DbuildNumber=da8f1b9b5a799bfa8e2d8aa9ab31d6d5a1cc31a0 -DskipTests -Dpython.ver="python >= 2.6"
