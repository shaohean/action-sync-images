...
RUN apt-get update > /dev/null && \
    ...
    echo ". ${CONDA_DIR}/etc/profile.d/conda.sh && conda activate base" >> ~/.bashrc && conda update conda -y && conda create -n myenv python=3.12 -y && 'export PATH="/opt/conda/bin:$PATH"' >> /root/.bashrc && echo 'conda activate myenv' >> /root/.bashrc && source /root/.bashrc
ENTRYPOINT ["tini", "--"]
CMD ["/bin/bash"]
